...
minday=datenum([2012 09 10 0 0 0]);
maxday=datenum([2012 09 15 0 0 0]);
...
[h,bar]=gplot2(log10(Pbgr2_ar(1:2000,:)),[], times, z(1:2000));
...
screen2png('lalinet_argentina.png');

figure(2); clf;
temp=get(gcf,'position'); temp(3)=900; temp(4)=300;
set(gcf,'position',temp); % units in pixels!
set(gca,'position',[0.07 0.12 0.84 0.75])  
[h,bar]=gplot2(log10(Pbgr2_ma(1:2000,:)),[], times, z(1:2000));
datetick('x',6, 'keeplimits')
grid on;box on;
ylabel('Range (km)');ylim([0.5 12])
ylabel(bar,'Log(P r^2) [a.u.]'); 
title('ma-MA');
prettify(gca,bar); grid on; 
screen2png('lalinet_manaus.png');

figure(3); clf;
temp=get(gcf,'position'); temp(3)=900; temp(4)=300;
set(gcf,'position',temp); % units in pixels!
set(gca,'position',[0.07 0.12 0.84 0.75])  
[h,bar]=gplot2(log10(Pbgr2_sp(1:2000,:)),[], times, z(1:2000));
datetick('x',6, 'keeplimits')
grid on;box on;
ylabel('Range (km)');ylim([0.5 12])
ylabel(bar,'Log(P r^2) [a.u.]'); 
...
prettify(gca,bar); grid on; 
screen2png('lalinet_saopaulo.png');

figure(4); clf;
temp=get(gcf,'position'); temp(3)=900; temp(4)=300;
set(gcf,'position',temp); % units in pixels!
set(gca,'position',[0.07 0.12 0.84 0.75])  
[h,bar]=gplot2(log10(Pbgr2_ch(1:2000,:)),[], times, z(1:2000));
datetick('x',6, 'keeplimits')
grid on; box on;
